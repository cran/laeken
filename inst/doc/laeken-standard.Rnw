...
%  Department of Statistics and Probability Theory, Vienna University of 
%  Technology\\
%  E-mail: \href{mailto:alfons@statistik.tuwien.ac.at}{alfons@statistik.tuwien.ac.at}
  ORSTAT Research Center, Faculty of Business and Economics, Katholieke 
  Universiteit Leuven\\
  E-mail: \href{mailto:andreas.alfons@econ.kuleuven.be}{andreas.alfons@econ.kuleuven.be}
  \end{tabular}
}
...
The add-on package \pkg{laeken} \citep{alfons11b} aims is to bring 
...
and implemented in the \proglang{R} package \pkg{simPopulation} \citep{kraft10}. 
...
\begin{description}
  ...
  \item[\code{valueByStratum}:] A \code{data.frame} containing the point estimates by stratum.
  ...
  \item[\code{varByStratum}:] A \code{data.frame} containing the variance estimates by stratum.
  \item[\code{ci}:] A numeric vector or matrix containing the confidence interval(s).
  \item[\code{ciByStratum}:] A \code{data.frame} containing the confidence intervals by stratum.
...
influenced by outliers \citep[see][]{hulliger09a, alfons10f}. Since the upper 
...
\citep[see][]{alfons10f}. Vignette \code{laeken-pareto} \citep{alfons11a} 
...
highly influenced by outliers \citep[see also][]{hulliger09a, alfons10f}. 
