...
%% almost as usual
\author{Andreas Alfons\\ Erasmus University Rotterdam,\\ KU Leuven \And 
        Matthias Templ\\Vienna University of Technology,\\ Statistics Austria}
...
% A more complete overview of the available functionality is given in three 
% additional package vignettes on specialized topics: 
% %
% \begin{description}
...
% Thus only the most important variables from the survey are available. 
The data are synthetically generated from preprocessed 
Austrian EU-SILC data from 2006 provided by Statistics 
Austria.  
% Most of the variable names are rather cryptic codes, but these are the 
% standardized names used by the statistical agencies. 
...
observations of \code{eusilc} are printed below.
<<>>=
data("eusilc")
head(eusilc, 3)
@
...
% earnings data included in the survey. 
...
% measure poverty, social cohesion and gender differences. 
% Unless otherwise stated, the presented definitions strictly follow 
% \citet{EU-SILC04, EU-SILC09}, where more details can be found. In addition to 
% the mathematical definitions of the indicators, quick examples for computing 
% them with package \pkg{laeken} are provided. A detailed discussion on the 
% respective functions is given later on in Section~\ref{sec:design}.
...
% First, Section~\ref{sec:class} describes the functions for estimating the 
% indicators and the class structure of the returned objects. 
% Sections~\ref{sec:sub} and~\ref{sec:subset} then show how to estimate 
% indicators for different subdomains and extract subsets of the information 
...
% \citep{chambers92}. 
Package \pkg{laeken} follows an object-oriented design using \proglang{S3} classes \citep{chambers92}. 
Thus each of the above functions returns an object 
...
% interest. 
% For this purpose, let us revisit the first example from the previous 
...
% with the corresponding \code{subset()} method. 
...
includes recently developed methods of \citet{alfons13} that allow sampling 
...
\ref{eq:quantiles} reduces to the theoretical quantiles taken on the $n$ 
inner grid points from $n+1$ equally sized subsets of the interval $[0,1]$ 
...
% Two promising estimators are discussed in the following. Both were adjusted for 
