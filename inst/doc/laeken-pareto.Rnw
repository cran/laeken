...
with the add-on package \pkg{laeken} \citep{alfons11b}. The basic design of the 
...
\pkg{simPopulation} \citep{kraft10}. More information on the example data can 
...
1}^{k} w_{n-k+i} \left( \log x_{n-k+i} - k \log x_{n-k} \right)} .
\end{equation}
