...
#        if(!is.numeric(X)) stop("'X' must be a numeric matrix")
    ...
    if(byYear) {
        ...
        # function(y, x, R, p, X, totals, rs, alpha, ciType, na.rm, ...)
        ...
        b <- boot(data, bootFun, R, strata=design, p=p, X=X, 
...
getBootFun <- function(calibrate, fun) {
    if(calibrate) {
        function(x, i, p, X, totals, rs, na.rm, ...) {
            x <- x[i, , drop=FALSE]
            X <- X[i, , drop=FALSE]
            g <- calibWeights(X, x$weight, totals, ...)
            x$weight <- g * x$weight
            fun(x, p, rs, na.rm)
        }
    } else {
    ...
    if(byStratum) {
        if(calibrate) {
            ...
            function(y, x, R, design, p, X, totals, ys, rs, alpha, ciType, na.rm, ...) {
                ...
                design <- design[i]
                totals <- totals[ys == y,]
                b <- boot(x, fun, R, strata=design, p=p, X=X, 
                    totals=totals, rs=rs, na.rm=na.rm, ...)
                var <- apply(b$t, 2, var)
